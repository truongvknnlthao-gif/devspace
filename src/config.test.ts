...
assert.match(
  loadConfig(baseEnv).deviceHelperPath,
  /Applications\/DevSpace Device Helper\.app\/Contents\/MacOS\/DevSpace Device Helper$/,
);
assert.equal(
  loadConfig({
    ...baseEnv,
    DEVSPACE_DEVICE_HELPER_PATH: join(emptyConfigDir, "custom-helper"),
  }).deviceHelperPath,
